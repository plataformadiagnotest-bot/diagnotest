      ...
      <Topbar
        ...
        actions={
          <button className="flex items-center gap-1.5 px-3.5 py-2 bg-g800 text-white text-[12px] font-medium rounded-[6px] hover:bg-g700">
            <i className="ti ti-plus" /> Nueva veterinaria
          </button>
        }
      />
      <div className="p-6">
            ...
            <table className="w-full border-collapse text-[12px]">
              <thead>
                <tr className="bg-gy50">
                  {["ID", "Nombre", "Código", "Zona", "Condición facturación", "Estado", ""].map((h) => (
              ...
              <tbody>
                {(vets ?? []).map((v) => (
                  <tr key={v.id} className="hover:bg-gy50 border-b border-gy100 last:border-0">
                    <td className="px-3.5 py-2.5 font-mono text-[11px] text-gy400">{v.id.slice(0, 8)}</td>
                    <td className="px-3.5 py-2.5 font-medium text-gy900">{v.nombre}</td>
                    <td className="px-3.5 py-2.5 font-mono text-g700">{v.codigo}</td>
                    <td className="px-3.5 py-2.5">{(v.zona as any)?.nombre ?? "—"}</td>
                    <td className="px-3.5 py-2.5 text-gy600">{v.condicion_facturacion ?? "—"}</td>
